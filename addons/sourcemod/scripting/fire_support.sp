...
public void OnPluginStart() {
	...
	gCvarEnableAllClasses = CreateConVar("sm_enable_all_classes", "1", "Enables all classes to use fire support.");
	...
	InitSupportCount();
	AutoExecConfig(true, "fire_support");
...
public Action Event_WeaponFire(Event event, const char[] name, bool dontBroadcast) {
	...
	//PrintToChatAll("IsEnabled[client]: %d, Team: %d, Count: %d, Enabled: %d",IsEnabled[client], team,CountAvailableSupport[team], IsEnabledTeam[team]);
	
	char indicator[64];
	gCvarWeapon.GetString(indicator, sizeof(indicator));
	if(IsEnabled[client] || GetConVarInt(gCvarEnableAllClasses))
	{
		if ((team != TEAM_SECURITY && team != TEAM_INSURGENT) || !IsPlayerAlive(client) || CountAvailableSupport[team] < 1 || !IsEnabledTeam[team] || (StrContains(weapon, indicator, false) == -1)) {
			return Plugin_Handled;
		}
	}
	else
	...
	if (GetAimGround(client, ground)) {
		...
		if (CallFireSupport(client, ground)) {
			...
			PrintToChatAll("Fire support incoming! Get cover!");
		}
	}
...
Action CmdCallFS(int client, int args) {													// HINT: Fire to where client looking
	...
	if(IsEnabled[client] || GetConVarInt(gCvarEnableAllClasses))
	{
		if ((team != TEAM_SECURITY && team != TEAM_INSURGENT) || !IsPlayerAlive(client) || CountAvailableSupport[team] < 1 || !IsEnabledTeam[team]) {
			return Plugin_Handled;
		}
	}
	else
...
public Action Timer_EnableTeamSupport(Handle timer, int team) {
	PrintToChatAll("Fire support available: %d remaining",CountAvailableSupport[TEAM_SECURITY]);
